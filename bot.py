...
log.info(f"State loaded: balance=${merged['balance']:.2f} wins={merged['wins']} l
return merged
except Exception as e:
...
json.dump({k: v for k, v in state.items() if k != "running"}, f, indent=2, except Exception as e:
...
sig = base64.b64encode(hmac.new(BITGET_API_SECRET.encode(), msg.encode(), hashlib.sh
headers = {
...
try:
r = bitget_signed("GET", f"/api/v2/mix/account/account?symbol={BITGET_SYMBOL}&produ
...
def get_ticker():
try:
r = bitget_get("/api/v2/mix/market/ticker", {"symbol": BITGET_SYMBOL, "productType": BITG
...
if bear: log.info(f"Bearish divergence: price {ph[-2]:.0f}→{ph[-1]:.0f} RSI {rh[-2]:.1f}→
if bull: log.info(f"Bullish divergence: price {pl[-2]:.0f}→{pl[-1]:.0f} RSI {rl[-2]:.1f}→
...
log.info(f"Box: date={yesterday} H={box['high']:.2f} L={box['low']:.2f} MID={box['mid']:.
state["box"] = box
return box
# ── NEWS ──────────────────────────────────────────────────────────
NEWS_SOURCES = [
...
Headlines:
{news_block}
Return ONLY JSON: {{"score": <-2 to +2>, "summary": "<max 20 words>", "key_factor": "<headlin
...
log.error(f"LONG wrong TP/SL - closing. entry={pos['entry']} tp={pos['tp']} sl={p
state["position"] = None
save_state()
return
if pos["type"] == "SHORT":
if pos["tp"] >= pos["entry"] or pos["sl"] <= pos["entry"]:
log.error(f"SHORT wrong TP/SL - closing. entry={pos['entry']} tp={pos['tp']} sl={
state["position"] = None
save_state()
return
...
pnl = round(pnl, 2)
log.info(f"Position closed: {'TP ' if hit_tp else 'SL state["pnl_total"] = round(state["pnl_total"] + pnl, 2)
'} | PnL={pnl:+.2f}")
state["balance"] = round(state["balance"] + pnl, 2)
...
f"PnL: {'+'if pnl>=0 else ''}${pnl:.2f}\n"
f"Balance: ${state['balance']:,.2f}\n"
f"W/L: {wins}W/{losses}L | WR: {round(wins/(wins+losses)*100) if wins+losses>0 )
...
state["last_signal"] = f"HOLDING {pos['type']} @ {pos['entry']:.2f} | PnL: else 0
{pnl:+
...
log.info(f"Price={price:.2f} RSI={rsi} Box=[{box['low']:.2f}–{box['high']:.2f}] MID={box[
f"BullDiv={bull_div} BearDiv={bear_div} | 4H Support={support:.2f} Resistance={r
...
log.info(f"SHORT check: price={price:.2f} PDH={box['high']:.2f} at_pdh={at_pdh} rsi={rsi}
...
log.info(f"SHORT: SL={sl} TP={tp} qty={qty} divergence={bear_div} risk={risk_pct*100:
...
log.info(f"LONG check: price={price:.2f} PDL={box['low']:.2f} at_pdl={at_pdl} rsi={rsi} r
...
qty = calc_qty(balance, risk_pct, price, sl)
log.info(f"LONG: SL={sl} TP={tp} qty={qty} divergence={bull_div} risk={risk_pct*100:.
headlines = fetch_news()
...
state["last_signal"] = f"WAIT | RSI={rsi} | Box [{box['low']:.0f}–{box['high']:.0f}] | {d
log.info(state["last_signal"])
# ── BOT LOOP ─────────────────────────────────────────────────────
def bot_loop():
log.info("=======================================")
...
log.info(f" Balance: ${state['balance']:.2f} | Wins: {state['wins']} Losses: {state['los
log.info("=======================================")
send_telegram(
...
state["errors"].append(f"{datetime.now(timezone.utc).strftime('%H:%M')} {str(e)[:
