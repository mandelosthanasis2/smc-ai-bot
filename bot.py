...
1. Entry on 1H timeframe
1. SHORT at PDH: RSI > 70, price within 0.5% of PDH
1. LONG  at PDL: RSI < 30, price within 0.5% of PDL
1. SL at nearest 4H S/R + 0.3% buffer (MAX 1.5% from entry)
1. TP at MID of box
...
# Indicators
# Include current live price as the last "close" for real-time RSI
# This matches what TradingView shows
closes = [c["close"] for c in candles_1h]
closes_live = closes + [price]   # add current live price
rsi = calc_rsi(closes_live)
log.info(f"RSI (live): {rsi} (closed candles only: {calc_rsi(closes)})")
